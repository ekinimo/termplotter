...
impl<Context> Eval<Range, Context, ExpressionRange1dResult> for DummyRange<ExpressionRange1dResult> {
    fn eval(tree: &Range, _context: &Context) -> Result<ExpressionRange1dResult, EvaluationError> {
        match tree {
            ...
            Range::NumericStep(x) => {
                ExpressionRange1dResult::create_with_step(x.value.1, x.value.3, x.value.2)
            }
            Range::FileBare(x) => ExpressionRange1dResult::create_from_file(x.clone().value.1)
                .map_err(|a| a(x.location.0, x.location.1)),
            Range::FileCol(x) => ExpressionRange1dResult::create_from_file_col(x.clone().value.1,x.clone().value.2)
                .map_err(|a| a(x.location.0, x.location.1)),
        }
    }
}
